                    ...
                    <div class="card shadow mb-4">
                        <div class="container p-3">
                                ...
                                <div class="btn-group">
                                    ...
                                    @if($student_id)
                                        <button type="button" class="btn btn-info btn-sm dropdown-toggle" data-bs-toggle="dropdown" aria-expanded="false">
                                            {{ ucfirst('Verifikasi Status Foto') }}
                                        </button>
                                        <ul class="dropdown-menu">
                                            <li>
                                                <form action="{{ route('update-media-status') }}" method="POST">
                                                    @csrf
                                    ...
                                    @else
                                        <button type="button" class="btn btn-secondary btn-sm" disabled>No Media</button>
                                    @endif
                                </div>
...
<script>
    ...
    $(document).ready(function() {
        $(".update-status").click(function(e) {
            ...
            let button = $(this).closest(".btn-group").find("button");

            $.ajax({
                url: "{{ route('update-media-status') }}",
                type: "POST",
                data: {
                    _token: "{{ csrf_token() }}",
                    id: mediaId,
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        button.removeClass("btn-warning btn-success btn-danger")
                              .addClass(response.status === 'approved' ? 'btn-success' :
                                        response.status === 'rejected' ? 'btn-danger' : 'btn-warning');
                    }
                },
                error: function(xhr) {
